...
step 3: 将title渲染到视频的中央（包括英文标题以适合更广泛的受众），从视频开始6秒左右后开始渲染，6秒左右淡入6秒左右淡出，显示持续时间6秒左右。考虑提前分析关键帧，避免卡顿黑屏，不需要绝对精确。是否可以先裁切一小块片段（例如裁切18秒左右视频），渲染后再和后面的片段进行无损合并。避免渲染整个4K长视频。
